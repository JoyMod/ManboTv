...
- 每完成一个重构模块，必须立刻执行一次模块级验收，禁止累计多个模块后再统一验证
- 验收必须基于 Docker 实际运行环境（`docker compose up -d --build`）
- 验收必须包含该模块核心接口的 `curl` 实测（至少覆盖成功链路，建议覆盖失败链路）
- 当前模块未通过前，不允许进入下一个模块改造
