...
- **规则**: 只要发生 bug 修复，无论改动在前端、后端还是配置层，都必须在 Docker 环境重建并重启相关服务后再验收
- **最低要求**:
  1. 执行 `docker compose up -d --build`，除非能明确证明只需重建单个服务
  2. 至少验证一条对应页面访问或接口请求，确认命中的是容器中的新版本
  3. 若 Docker 构建失败，必须先修复构建或部署问题，再结束本次任务
